...
def test_run_package_tests_returns_true_on_zero_exit() -> None:
    ...
    with patch(
        "agentic_v2.devex.workspace_test_runner.subprocess.run",
...
def test_run_all_returns_true_when_all_pass() -> None:
    ...
    with patch(
        "agentic_v2.devex.workspace_test_runner.subprocess.run", return_value=passing
    ):
        result = run_all()
...
def test_run_all_package_filter_runs_only_named_package() -> None:
    ...
    called_cwd = mock_run.call_args.kwargs.get("cwd") or mock_run.call_args[1].get(
        "cwd", ""
    )
