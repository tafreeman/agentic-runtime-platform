...
Covers :func:`agentic_v2.server.lifespan._enforce_rate_limiting_available`, the
...
under test. The flags are patched on the ``lifespan`` module because that is
where the guard reads them (the server-module split moved the guard out of
``app.py`` into ``lifespan.py``).
"""
...
from agentic_v2.server import lifespan as lifespan_module
from agentic_v2.server.lifespan import _enforce_rate_limiting_available
...
class TestRateLimitingStartupGuard:
    ...
    def test_raises_when_slowapi_unavailable_and_no_override(
        ...
        monkeypatch.setattr(lifespan_module, "_SLOWAPI_AVAILABLE", False)
        monkeypatch.setattr(lifespan_module, "_RATE_LIMIT_DISABLED", False)
    ...
    def test_does_not_raise_with_legacy_disable_flag(
        ...
        monkeypatch.setattr(lifespan_module, "_SLOWAPI_AVAILABLE", False)
        monkeypatch.setattr(lifespan_module, "_RATE_LIMIT_DISABLED", True)
        monkeypatch.delenv("AGENTIC_DISABLE_RATE_LIMITING", raising=False)
    ...
    def test_does_not_raise_when_slowapi_available(
        ...
        monkeypatch.setattr(lifespan_module, "_SLOWAPI_AVAILABLE", True)
        monkeypatch.delenv("AGENTIC_DISABLE_RATE_LIMITING", raising=False)
