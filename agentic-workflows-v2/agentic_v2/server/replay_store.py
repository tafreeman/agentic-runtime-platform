...
class ReplayStore(Protocol):
    ...
    implementations must catch their own errors and never propagate them to
    the broadcast path.
    """
...
class RedisReplayStore:
    ...
    def _handle_connection_loss(self, exc: BaseException) -> None:
        ...
        if isinstance(exc, (RedisConnectionError, OSError)):
            logger.error(
                "RedisReplayStore: connection lost, replay persistence disabled: %s", exc
            )
            self._connected = False
...
class SqliteReplayStore:
    ...
    async def connect(
        ...
        store = cls(db_path=db_path, max_events=max_events)
    ...
    async def append(self, run_id: str, event: dict[str, Any]) -> None:
        """Append *event* and enforce the per-run event cap.
    ...
    async def clear(self, run_id: str) -> None:
        ...
        try:
            ...
            logger.warning(
                "SqliteReplayStore.clear failed for run %s: %s", run_id, exc
            )
...
async def _build_auto_store(
    ...
    if _SQLITE_AVAILABLE:
        try:
            ...
            logger.warning("ReplayStore: SQLite init failed (%s), using memory", exc)
