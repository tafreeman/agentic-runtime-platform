...
| Next | `run_wave.py --wave 12 --size 16` — the wave number is the highest existing `dataset/cases.swebench.wave*.jsonl` plus one (wave 11 is graded and landed, so 12 as of 2026-09-02). Never reuse a graded wave's number: `run_wave.py` refuses `--rebuild-cases` once that wave's reports exist, because `run_ab.py` would overwrite them. A new wave is never-before-graded by construction — `build_swebench_cases.py` skips every instance a tracked manifest or report has already drawn, not only the ones with a local case directory (this campaign stopped reusing the closed segment's instances, 2026-08-30). **Check the resulting report's `manifest.target_fingerprint`** before unioning it with wave 8+9 or wave 10+11 — Ollama can update the served model underneath the same tag name (EVIDENCE.md §2.21). |
