...
| **052** | Optional typed step artifact contracts shared by both engines; `code_artifact` validates file maps/sentinel payloads, promotes only independently valid legacy aliases, and makes fullstack backend handoffs fail closed without changing `coalesce()` | Accepted | [ADR-052](ADR-052-typed-step-artifact-contracts.md) |
...
| **058** | Claude subscription backend — `ClaudeSubscriptionBackend` behind a new `claude:` prefix, registered by *capability* (`claude-agent-sdk` importable) rather than by API key, so a subscription-only operator finally gets a Claude backend instead of an Ollama-only fallback; `subscription_env()` blanks `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` in the CLI subprocess because `agentic_v2.models.secrets` puts them in `os.environ` and the child would otherwise silently bill the API account; also repairs `ClaudeSDKAgent`, which raised `AttributeError` on every run (`message.type` on dataclasses that have no `.type`) | Accepted | [ADR-058](ADR-058-claude-subscription-backend.md) |
| **059** | Curated cost lane + fail-closed failover ceiling — `cost_lane` (`local`/`free`/`paid`) added to every registry entry; `AGENTIC_MAX_COST_LANE` filters (not reorders) `get_model_candidates_for_tier`'s full candidate list including a pinned `model_override`, raising `CostLaneCeilingExceededError` rather than falling through unfiltered when every candidate is filtered out; `_invoke_with_failover` logs + records a `lane_crossings` run event whenever failover moves into a strictly more expensive lane. Closes the gap the SWE-fix A/B campaign hit reaching Anthropic on a run meant to spend nothing | Accepted | [ADR-059](ADR-059-model-cost-lane-ceiling.md) |
| **060** | One set of scheduling rules, proved in Lean 4 — `DAGExecutor` and the orchestrator's `_execute_plan` share one rule set: a parallel limit of at least 1, plans validated before anything runs (LLM-written plans included), a failed step's dependents skipped with a recorded reason, and success only when nothing failed or was dropped (the deadlock branch now fails the run). A Lean model of `DAGExecutor` in `proofs/` (CI-only) with a replay test tying it to the code; bounded parallelism and honest status proved, safety, no duplicate starts and completeness open (see `proofs/README.md`) | Accepted | [ADR-060](ADR-060-scheduler-guarantees-lean.md) |
**Note:** ADRs 004-006 and 013 were never created or were withdrawn; those numbering gaps are intentional and should not be reclaimed. ADR-049 and ADR-053 are claimed by parked or in-flight work (ADR-049 by a noncompensatory-gates draft in a stashed working set; ADR-053 by the memoryctl branch behind PR #205) — do not reclaim those numbers either. (The decision once numbered ADR-013 in the `agentic-systems-lab` fork was salvaged into this repo as **ADR-031**, not as 013.) ADR-023 working notes (migration plan, phase tracker, finish plan, divergence audit, preservation matrix) were moved to [`drafts/`](drafts/README.md) on 2026-06-17 to resolve a naming collision; the canonical decision record remains at ADR-023.
