...
- **Files under 800 lines** — split before a module grows past this. Existing files above the guide are tracked in the exception register ([ADR-053](adr/ADR-053-file-size-exception-register.md)); a file may exceed the guide only while it is listed there.
