...
| `AGENTIC_BLOCK_PRIVATE_IPS` | `1` (default **ON**) | No | When `1` (or unset), HTTP tool requests that resolve to private, loopback, link-local, or reserved IP ranges are rejected. Also resolves DNS names and blocks if any returned address is private. Redirects are re-validated at each hop. Set to `0` only when workflows legitimately reach internal services and you have applied compensating network controls. Implemented in `agentic_v2/security/url_guard.py`. |
