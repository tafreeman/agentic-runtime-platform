...
PRs should explain intent and verification, link relevant issues, include screenshots for UI changes, update tests/docs and `CHANGELOG.md` when user-visible, and add an ADR for material architectural decisions. Before requesting review, run `just test`, `just docs`, and all pre-commit hooks. Never commit `.env`, provider tokens, generated run data, or other secrets.
