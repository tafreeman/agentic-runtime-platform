...
4. **Close the consensus fail-open** — missing/`None` `min_agreement` must not coerce to 0.0 (`engine/agent_resolver.py:225-229`), and `None` votes must count as failure rather than valid consensus candidates; regenerate `datasets/default/consensus_review_output.json` (the committed golden currently shows unanimous "consensus" over three null verdicts).
