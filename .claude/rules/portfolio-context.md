...
- Coverage uses `precision = 2` (79.93% fails) and `[tool.coverage.run] omit` excludes ~20 modules (all of `agents/implementations`, `prompts`, `models/client.py`, `backends_*`, the three `ek_*` bridges, `workflows/runner.py`…) — code added there cannot help you clear the gate.
...
- UI coverage `branches` floor is **56**, not 60, and `ui/vitest.config.ts` declares it a one-way ratchet.
- `docs/adr/ADR-INDEX.md` is authoritative for the next free ADR number (004-006 and 013 are unused); `CONTRIBUTING.md` §6 is stale on this, and §4 credits a nonexistent `scripts/generate_schemas.py`.
- Tests must use `monkeypatch.setenv`; the `_snapshot_os_environ` autouse fixture lives in `agentic-workflows-v2/tests/conftest.py` — **not** a repo-root `tests/conftest.py`, which does not exist despite what `.claude/rules/testing.md` says.
...
- `AGENTIC_NO_LLM=1` is the zero-credential baseline (swaps in `PlaceholderChatModel`); it does **not** change engine selection.
- `ci.yml` defines **17** blocking jobs; `eval-package-ci.yml`'s `eval-golden-gate` is a required check on `main` (hence no paths filter). Nightly 50× streaming + flake/SLO gates block a *release cut*, not a PR. `manifest-temperature-check.yml` is dead — both its script and its path-filter file are absent.
